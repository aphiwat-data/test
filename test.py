...
def _normalize_stay_month(raw) -> str | None:
    ...
    if m:
        month_part, year_part = m.group(1).strip().title(), m.group(2).strip()
        try:
            yr = int(year_part)
            if yr < 100:
                yr += 2000
...
def _build_hotel_table(df, hotel, pivot2_months, ref_col_map, short_names, month_order_p2, ref_order_p2, metric_short_p2):
    """Build pivot table and styler for one hotel. Returns styler or None if no data."""
    month_slice = df[(df["Stay Month"].isin(pivot2_months)) & (df["Hotel"] == hotel)]
    rows_p2 = []
    for _, r in month_slice.iterrows():
        month = r["Stay Month"]
        for ref_label, metric_cols in ref_col_map.items():
            row = {"Month": month, "Reference": ref_label}
    ...
    pivot_swapped_p2 = melt_p2.pivot_table(
        index=["Month", "Metric"], columns="Reference", values="Value"
    ).reset_index()
    pivot_swapped_p2["Metric"] = pivot_swapped_p2["Metric"].map(metric_short_p2)
    pivot_swapped_p2["Month"] = pd.Categorical(pivot_swapped_p2["Month"], categories=month_order_p2, ordered=True)
    pivot_swapped_p2["Metric"] = pd.Categorical(pivot_swapped_p2["Metric"], categories=["Occ", "Room", "ADR", "Rev"], ordered=True)
    ...
    mask_occ = pivot_swapped_p2["Metric"] == "Occ"
    for c in ref_cols_p2:
        pivot_swapped_p2.loc[mask_occ, c] = pivot_swapped_p2.loc[mask_occ, c].apply(
    ...
    def _duetto_rev_style_p2(row):
        ...
        if pivot_swapped_p2.loc[row.name, "Metric"] != "Rev" or "Duetto" not in ref_cols_p2 or "Budget" not in ref_cols_p2:
            return out
        idx = row.name
        d, b = pivot_swapped_p2_numeric.loc[idx, "Duetto"], pivot_swapped_p2_numeric.loc[idx, "Budget"]
    ...
    return pivot_swapped_p2.style.apply(_duetto_rev_style_p2, axis=1, subset=ref_cols_p2).set_properties(
        subset=ref_cols_p2, **{"text-align": "right"}
    )
...
def main():
    ...
    with col_left:
        uploaded = st.file_uploader("Upload report (CSV)", type=["csv"], key="pivot2_upload")
    ...
    if "pivot2_df_cache" not in st.session_state or st.session_state.pivot2_df_cache_key != cache_key:
        df, title, generated = parse_uploaded_report(uploaded)
        df = df.rename(columns={df.columns[0]: "Hotel", df.columns[1]: "Stay Month"})
        # Normalize Stay Month to canonical "Jan, 2026" (handles Jan-26, Jan,26, etc.)
        df["Stay Month"] = df["Stay Month"].apply(lambda x: _normalize_stay_month(x) or x)
    ...
    all_hotels = [h for h in df["Hotel"].dropna().unique() if h and str(h).strip() and h != "Total"]
    month_order_p2 = [
        "Jan, 2026", "Feb, 2026", "Mar, 2026", "Apr, 2026", "May, 2026", "Jun, 2026",
        "Jul, 2026", "Aug, 2026", "Sep, 2026", "Oct, 2026", "Nov, 2026", "Dec, 2026",
    ]
    months_avail_p2 = [m for m in month_order_p2 if m in df["Stay Month"].values]
    this_month = datetime.now().strftime("%b, %Y")
    default_months = [this_month] if this_month in months_avail_p2 else (months_avail_p2[:1] if months_avail_p2 else [])
    with col_left:
        pivot2_months = month_placeholder.multiselect("Stay Month", options=months_avail_p2, default=default_months, key="pivot2_months")
    with col_right:
        selected_hotels = hotel_placeholder.multiselect("Hotels", options=all_hotels, default=all_hotels, key="pivot2_hotels", placeholder="All hotels")
        view_mode = view_placeholder.radio("View", options=["Tab view", "List view"], index=0, key="pivot2_view", horizontal=True)
    ...
    # Build column map once (ref_label -> {short_name: col_name}) to avoid repeated lookups
    ...
    ref_order_p2 = ["Today", "STLY", "ST2Y", "ST3Y", "Duetto", "Budget", "Final LY", "Final 2Y", "Final 3Y"]
    metric_short_p2 = {"Occupancy (%)": "Occ", "Rooms": "Room", "ADR": "ADR", "Revenue": "Rev"}
    ...
    if view_mode == "Tab view":
        tab_labels = [HOTEL_SHORT_NAMES.get(h, h) for h in selected_hotels]
        tabs = st.tabs(tab_labels) if tab_labels else []
        for tab, hotel in zip(tabs, selected_hotels):
            with tab:
                styler = _build_hotel_table(df, hotel, pivot2_months, ref_col_map, short_names, month_order_p2, ref_order_p2, metric_short_p2)
                render_table(styler)
    else:
